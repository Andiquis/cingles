...
if command -v php >/dev/null 2>&1 && command -v sqlite3 >/dev/null 2>&1; then
    ...
    if [ ! -f "$DB_PATH" ]; then
        echo "Base de datos vocabulario.db no encontrada. Creándola..."
        php "$HOME/eureka/database.php"
        echo "Base de datos vocabulario.db creada."
    else
        echo "Base de datos vocabulario.db ya existe."
    fi
